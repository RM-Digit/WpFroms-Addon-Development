    ...
    public function download_csv_file(){
        ...
        if( isset($_REQUEST['wpforms-csv']) && isset( $_REQUEST['nonce'] ) ){
            ...
            for( $p = 0; $p <= $total_query; $p++ ){
                ...
                $reqData = str_replace("-*-"," ",$_REQUEST['reqData']);
                $reqData_key = explode("99",$reqData)[0];
                $reqData_val = explode("99",$reqData)[1];
                $data  = array();
                $i     = 0;
                $ii = 0;
                $can = 0;
                foreach ($results as $result) :
                    ...
                    $resultComp = array_values( $resultTmp);
                 
                    if(empty($reqData) || empty($reqData_val)){
                        $can = 1;
                    }else{
                        for($ii =0; $ii < count($heading_key); $ii++ )
                        {
                            $key_i    =  array_search($reqData_key,$heading_key);
                            if(strpos(strtoupper($resultComp[$key_i]),strtoupper($reqData_val)) !== false || strtoupper($resultComp[$key_i])== strtoupper($reqData_val)){
                                $can = 1;
